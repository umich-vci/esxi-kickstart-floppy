...
class Floppy:
    ...
    def _filestring(s, length):
        ...
        if len(b) != length:
            raise s.Error("File string '%s' too long? (%d != %d)" % (s,len(b),length))
        return b
    ...
    def _boot_open(self):
        ...
        if (len(self.data) < 38):
            raise self.Error("Not enough data in image for boot sector. (%d bytes)" % len(self.data))
